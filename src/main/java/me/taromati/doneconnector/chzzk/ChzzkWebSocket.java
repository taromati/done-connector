...
//                    Logger.info(ChatColor.RED + "치지직 웹소켓 핑 스레드가 종료되었습니다.");
    ...
    public void onMessage(String message) {
        Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] onMessage: " + message);

        try {
            JSONObject messageObject = (JSONObject) parser.parse(message);

            Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] 파싱 시작");
            ...
            if (cmd == CHZZK_CHAT_CMD_PING) {
                ...
                Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] ping");

                return;
            }
            if (cmd == CHZZK_CHAT_CMD_PONG) {
                Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] pong");

                return;
            }
            if (cmd != CHZZK_CHAT_CMD_DONATION) {
                Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] 도네이션 아님");
            ...
            Logger.debug(ChatColor.WHITE + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] 파싱 완료");
            ...
            Logger.error(ChatColor.RED + "[ChzzkWebsocket][" + chzzkUser.get("nickname") + "] 치지직 메시지 파싱 중 오류가 발생했습니다.");
            Logger.debug(ChatColor.LIGHT_PURPLE + e.toString());
        }
    }
