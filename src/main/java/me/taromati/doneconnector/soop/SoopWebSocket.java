...
public class SoopWebSocket extends WebSocketClient {
        ...
        pingThread = new Thread(() -> {
            ...
            while (isAlive) {
                try {
                    ...
                    Logger.info(ChatColor.RED + "숲 웹소켓 핑 스레드가 종료되었습니다.");
                }
            }
        });
    ...
    public void onMessage(ByteBuffer bytes) {
        Logger.debug(ChatColor.WHITE + "[SoopWebSocket][" + soopUser.get("nickname") + "] onMessage: " + new String(bytes.array(), StandardCharsets.UTF_8));
        ...
        try {
            ...
            Logger.error(ChatColor.RED + "[SoopWebsocket][" + soopUser.get("nickname") + "] 숲 메시지 파싱 중 오류가 발생했습니다.");
